...
「**聞く**（音声認識）」「**訳す**（翻訳）」「**話す**（音声合成）」ができます。マイクの音声を聞き取って文字にし、日本語⇔英語に翻訳し、文字をいろいろな**キャラ声**（ふつう・ねずみ・ロボット・おばけ）で読み上げられます。声は**話す速さ**と**声の高さ**の倍率でチューニングできます。
...
| ブロック | 説明 |
| --- | --- |
| `[TEXT] と話す` | 文字を読み上げ、終わるまで待ちます。 |
| `声を [PRESET] にする` | キャラ声を選びます（ふつう / ねずみ / ロボット / おばけ）。 |
| `話す速さを [RATE] 倍にする` | プリセットに対する速さの倍率（1 = プリセットの標準）。 |
...
| `話す言語を [LANGUAGE] にする` | 読み上げ（話す）の言語（初期値：日本語）。 |
| `話す時の翻訳を [ON/OFF] にする` | **ON** にすると、`話す` のとき「**聞き取る言語 → 話す言語**」へ自動的に翻訳してから読み上げます（初期値：**OFF**）。 |
...
...
聞き取る
( 聞き取った言葉 ) と話す      ← 自動で日本語→英語に翻訳して英語で読み上げ
```
...
> **キャラ声について：** キャラ声プリセットは、**音声の再生速度（playbackRate）で高さと速さを同時に変化**させて表現します。この方式は**Mac・iPad・Chromebook を含むどの端末でも同じように効きます**（端末による差はありません）。`話す速さ` `声の高さ` のブロックも、この再生速度に反映されます。

> **翻訳について（重要）：** 翻訳には無料の [MyMemory](https://mymemory.translated.net/) API（APIキー不要）を使用しています。**ネット接続が必要**です。MyMemory には **1日あたりの無料利用上限（IP単位）** があり、匿名利用で約 5,000 語/日、メールアドレスを設定すると約 50,000 語/日に増えます。**学校などでは複数の端末が同じIPになる（NAT）ため、使いすぎると上限に達して翻訳できなくなる**ことがあります。上限を超えると `翻訳する` ブロックは空文字を返します。上限を引き上げたい場合は `src/vm/extensions/block/index.js` の `MYMEMORY_EMAIL` にメールアドレスを設定してください（さらに必要な場合は有料キーや自前プロキシへの差し替えも可能です）。
